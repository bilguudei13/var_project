...
\asmfield{Source in code}
\code{garch\_evt.py:227}: \lstinline|train = r_p.iloc[:t] * scale_factor|;
\code{garch\_evt.py:235}: \lstinline|mu_hat = res.params["mu"] / scale_factor|;
\code{garch\_evt.py:242}: \lstinline|sigma_sq_curr = fcast.variance.iloc[-1, 0] / scale_factor2|.
(Hardcoded constants replaced by \code{scale\_factor}/\code{scale\_factor2}
per ALIGN-001; see \cref{asm:F5} for the conditional scaling assumption.)
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{F.5 --- Conditional \%-scaling preserves \texttt{arch} numerical stability}\label{asm:F5}

\asmfield{Statement}
The conditional scaling rule
$\mathrm{scale\_factor} = 100$ if $\mathrm{std}(r_p) < 0.1$, else $1$,
preserves numerical accuracy of the \texttt{arch} library's MLE optimizer
for returns of widely varying magnitude.  For this project's portfolio
returns ($\mathrm{std}(r_p) \approx 0.011$), the branch always resolves to
$\mathrm{scale\_factor} = 100$.

\asmfield{Source in code}
\code{GARCH.py:89--91} (teammate's reference, authoritative per the
alignment audit):
\begin{lstlisting}
scale_factor = 1.0
if series.std() < 0.1:
    scale_factor = 100.0
\end{lstlisting}
\code{garch\_evt.py:196--197} (aligned via ALIGN-001,
\texttt{docs/audit/garch\_alignment\_report.md}):
\begin{lstlisting}
...
\asmfield{Theoretical basis}
The \texttt{arch} library's Student-$t$ GARCH MLE is ill-conditioned on
raw decimal returns of order $10^{-2}$ because the log-likelihood surface
becomes numerically flat near the optimum. Scaling by $100$ raises the
effective gradient magnitude to a regime where the optimizer converges
cleanly. See \texttt{arch} library documentation and the numerical stability
discussion in \cite{Bollerslev1986} (parameter estimation conventions).

\asmfield{Validation in this project}
\validated\ via the GARCH alignment audit
(\texttt{docs/audit/garch\_alignment\_report.md}, \S{}4):
post-alignment \texttt{garch\_evt.py} reproduces \texttt{GARCH.py}
parameter estimates to machine precision ($\mathrm{abs\_diff} = 0$ for
all of $\mu, \omega, \alpha_1, \beta_1, \nu$, log-likelihood, and the
entire conditional-volatility series). Because $\mathrm{std}(r_p) =
0.0112 < 0.1$, the conditional always resolves to $\mathrm{scale} = 100$
on this data.

\asmfield{Consequence if assumption fails}
If the scale factor were wrongly set to $1$ for returns of order
$10^{-2}$, the optimizer may misconverge or report spurious parameter
values; direction and magnitude of bias are not analytically tractable
because optimizer failure is discontinuous.  No real-world incident
documented; the assumption is industry-standard for \texttt{arch} use.

\asmfield{Mitigation in this project}
Identical scaling logic applied across both Python GARCH files
(\texttt{GARCH.py} and \texttt{garch\_evt.py}), verified by the alignment
audit to produce bit-identical outputs. The $\mathrm{std} < 0.1$ threshold
is conservative; the $\mathrm{scale} = 1$ branch handles correctly
percentage-quoted series not exercised in this project.
...
\cref{asm:F5}  & Conditional \%-scaling (\texttt{arch} stability) & Validated (numerical audit, 0 deviation) & Optimizer may misconverge if scale wrong & Discontinuous; never occurred & Identical scaling both Python GARCH files \\[4pt]
...
Conditional \%-scaling (\cref{asm:F5}) &
Re-run \code{garch\_evt.py} with \code{scale\_factor} forced to $1$
on portfolio returns; verify that the optimizer either fails gracefully
(convergence warning) or produces clearly wrong parameters
(e.g.\ $\hat{\omega} \approx 10^{-6}$, VaR implausibly small);
confirms that the $\mathrm{std} < 0.1$ branch is genuinely necessary &
15 min \\[4pt]
...
E (backtesting): 4; F (numerical): 5. Total: \textbf{41}.

\textbf{2.\ Validation status:}
Formally validated: 9 (A.6, A.7, B.3, B.7, B.8, C.6, D.3, D.6, E.4 --- plus
F.1, F.2, F.5 = 12 total validated). Diagnostic only: 16. NOT validated: 13.
...
(A.1--A.7, B.1--B.8, C.1--C.10, D.1--D.7, E.1--E.4, F.1--F.5) appear
