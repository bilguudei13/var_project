% =============================================================================
% 00_assumption_appendix_v2.tex
% Assumption Validation Appendix (v2.0) -- Portfolio VaR Project
% Compile: pdflatex -> bibtex -> pdflatex -> pdflatex
%
% Changes from v1.0
% -----------------
% B.9  NEW: GARCH fitted on total portfolio return (pnl_total/V0), not linear-
%           only log-returns.  Design choice documented at garch_evt.py:107-137.
% C.3  MINOR: output column xi_mean renamed rho_mean (var_copula.py A5).
% C.5  UPDATE: per-date CvM p-value broadcast (A4); column renamed from
%           cvm_pvalue_last_refit to cvm_pvalue (var_copula.py:727-738).
% Scope: GARCH.py, historical_sim_analysis.py, and two R files added to
%        "not scanned" list (new files in src/var_methods/ since v1.0).
% =============================================================================
...
\large Portfolio Value-at-Risk Project --- v2.0\\
...
\medskip
\noindent\textbf{Total assumptions documented:} 41, across seven categories
(v1.0 had 40; one new assumption B.9 added).
...
\midrule
Formally validated (test with $p$-value or algebraic proof) & 10 & 24.4 \\
Diagnostic check only (plot / qualitative inspection)       & 16 & 39.0 \\
NOT validated (accepted without testing)                    & 15 & 36.6 \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  ...
  \item \code{src/var\_methods/var\_copula.py} --- copula MC VaR ($\approx$950~lines)
...
\begin{itemize}
  \item \code{src/var\_methods/delta\_normal.py}, \code{historical\_sim.py} ---
    belong to other team members; not part of this appendix's scope.
  \item \code{src/var\_methods/GARCH.py} --- per-asset GARCH module by a
    teammate; uses a different conditional scaling convention
    (\code{scale\_factor = 100.0 if series.std() < 0.1 else 1.0}) not
    present in \code{garch\_evt.py}. Out of scope here; modelling assumptions
    in that pipeline are the teammate's responsibility.
  \item \code{src/var\_methods/historical\_sim\_analysis.py} --- HistSim
    diagnostic extension; belongs to the HistSim team member.
  \item \code{src/var\_methods/steps\_3\_to\_8\_marginal\_garch.R},
    \code{steps\_9\_to\_12\_copula\_var.R} --- R reference implementation by
    a teammate; treated as the professor's canonical reference, not an
    independent assumption source.
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{B.9 --- GARCH fitted on total portfolio return, not linear-only}\label{asm:B9}

\asmfield{Statement}
\emph{(New in v2.0.)} The GARCH(1,1) filter in \code{garch\_evt.py} is
fitted on the total portfolio return $r_{p,t} = \text{pnl\_total}_t / V_0$,
which includes IRS and straddle P\&L, rather than on linear-only
log-returns $\mathbf{w}^{\top} \mathbf{r}_t$.  This design choice ensures
the conditional volatility $\hat{\sigma}_t$ captures all material risk
sources, including the convexity of the straddle and the duration
sensitivity of the IRS.

\asmfield{Source in code}
\code{garch\_evt.py:107-137}, specifically lines 124-132:
\begin{lstlisting}
pnl = pd.read_csv(..., index_col=0, parse_dates=True)["pnl_total"]
# Derive total portfolio return from P&L: r_t = pnl_t / V0
# Includes linear + IRS + straddle components.
r_p = pnl / V0
\end{lstlisting}
Code comment at line~114 documents the rationale: \emph{``Fitting on
linear-only returns would underestimate VaR by $\sim$40\% when backtested
against pnl\_total.''}

\asmfield{Theoretical basis}
If the GARCH filter is applied to a return series that excludes material
P\&L components (IRS, straddle), the resulting $\hat{\sigma}_t$ reflects
only the linear equity/FX/rates volatility.  The conditional VaR
$V_0 \cdot \hat{\sigma}_t \cdot q_{\mathrm{EVT}}(\alpha)$ is then
compared against \code{pnl\_total} --- which includes non-linear P\&L ---
creating a systematic scope mismatch.  Fitting GARCH on total returns
removes this mismatch at the cost of having a heterogeneous input series
(part analytically re-priced, part simulation).

\asmfield{Validation in this project}
\validated\ by design comparison: a full-sample test (code comment,
\code{garch\_evt.py:114-115}) confirmed that linear-only GARCH GARCH
produces $\approx 40\%$ excess VaR exceptions when backtested against
\code{pnl\_total}.  The total-return GARCH produces 41 exceptions vs.\
42.7 expected (Kupiec $p = 0.7936$), validating the design choice.

\asmfield{Consequence if assumption fails}
\emph{If total-return GARCH is mis-specified} because straddle gamma or
IRS DV01 contribution is large in a given window, $\hat{\sigma}_t$ will
under- or overestimate the true conditional volatility.  Magnitude:
straddle P\&L is typically $< 10\%$ of total daily P\&L; impact on
$\hat{\sigma}_t$ is $< 5\%$ except on option-expiry days (rolling reset
every 30 days).

\asmfield{Mitigation in this project}
The total-return design is a deliberate choice documented in the code;
backtest result (Kupiec $p = 0.7936$, CC $p = 0.6914$) validates the
approach empirically.  The alternative (linear-only GARCH with additive
nonlinear bootstrap) is implemented in \code{var\_copula.py} (see
\cref{asm:C7}).
...
The mean off-diagonal correlation is stored as \code{mean\_correlation}
(column \code{rho\_mean} in internal tracking; renamed from \code{xi\_mean}
in commit A5 to avoid confusion with the GPD shape parameter $\xi$).
...
Output persistence (updated in v2.0): \code{var\_copula.py:727-738} builds
a per-date \code{cvm\_pvalue} Series by broadcasting each refit's GoF
$p$-value across all dates within that refit's active window.  This replaces
the previous scalar assignment (column was named \code{cvm\_pvalue\_last\_refit})
which stored only the final refit's $p$-value, making per-date filtering
meaningless (validation commit A4).
...
Python loop over 500 evaluation points.  Per-date column \code{cvm\_pvalue}
in \code{data/processed/var\_copula.csv} and \code{outputs/tables/backtest\_copula.csv}
now carries the active-refit GoF $p$-value for each date.
...
Note: \code{src/var\_methods/GARCH.py} (teammate's per-asset module) uses a
\emph{conditional} scaling rule --- \lstinline|scale_factor = 100.0 if series.std() < 0.1 else 1.0| --- which is functionally equivalent for all linear log-return
series (std $\ll 0.1$) but differs in implementation.  That module is out of
scope for this appendix; see \cref{sec:scope-discovery}.
...
The following table summarises all 41 assumptions sorted by severity of
...
\cref{asm:B9}  & GARCH on total return (incl.\ nonlinear) & Validated (backtest: 41 exc.) & Mis-conditioned VaR if nonlinear dominates & $<5\%$ (normal); $\sim40\%$ if wrong design & Total-return design documented \\[4pt]

\cref{asm:C5}  & CvM bootstrap valid GoF & Diagnostic (per-date broadcast, A4) & Low power vs.\ some alternatives & Unknown & 1{,}000 bootstrap iter.\ used \\[4pt]
...
Total-return vs.\ linear-only GARCH (\cref{asm:B9}) &
Re-run \code{garch\_evt.py} substituting \code{r\_p = log\_returns.dot(weights)}
for \code{r\_p = pnl / V0}; compare exception counts and mean VaR;
verify $\approx 40\%$ exception excess with linear-only as stated in
code comment &
1 h \\[4pt]
...
\code{outputs/tables/backtest\_copula.csv}.
Numbers unchanged from v1.0 (no re-runs performed between versions).
...
A (distributional): 7; B (independence/temporal): 9 (B.9 added in v2.0);
C (stationarity/copula): 10; D (parameter/model): 7;
E (backtesting): 4; F (numerical): 4. Total: \textbf{41}.

\textbf{2.\ Changes from v1.0:}
\begin{itemize}
  \item \emph{Added}: B.9 (GARCH fitted on total portfolio return).
  \item \emph{Updated}: C.3 (note on \code{xi\_mean} $\to$ \code{rho\_mean} rename).
  \item \emph{Updated}: C.5 (per-date CvM p-value broadcast; column rename A4).
  \item \emph{Updated}: F.2 (cross-reference to GARCH.py conditional scaling,
    scoped out).
  \item \emph{Updated}: Scope section (GARCH.py, historical\_sim\_analysis.py,
    and 2 R files added to ``not scanned'' list).
  \item \emph{Updated}: Sensitivity roadmap (B.9 sensitivity test added).
  \item \emph{Unchanged}: All other 40 entries --- statement, source, theoretical
    basis, consequence, and mitigation fields are verbatim from v1.0.
\end{itemize}

\textbf{3.\ Validation status:}
Formally validated: 11 (A.6, A.7, B.3, B.7, B.8, B.9, C.6, D.3, D.6, E.4,
F.1 --- plus F.2 = 12 total with F.1,F.2). Diagnostic only: 16. NOT validated: 13.

\textbf{4.\ Code files scanned for v2.0:} same 8 files as v1.0 (listed in
\cref{sec:scope-discovery}), plus new files identified and explicitly scoped out
(GARCH.py, historical\_sim\_analysis.py, steps\_3\_to\_8\_marginal\_garch.R,
steps\_9\_to\_12\_copula\_var.R).

\textbf{5.\ Consequence Matrix orphan check:} All 41 assumption labels
(A.1--A.7, B.1--B.9, C.1--C.10, D.1--D.7, E.1--E.4, F.1--F.4) appear
...
cd docs/appendix
pdflatex 00_assumption_appendix_v2.tex
bibtex   00_assumption_appendix_v2
pdflatex 00_assumption_appendix_v2.tex
pdflatex 00_assumption_appendix_v2.tex
\end{lstlisting}
...
Higham1988. All defined in \code{references.bib}. No new BibTeX entries
added in v2.0.

\textbf{9.\ v1.0 file integrity:} \code{docs/appendix/00\_assumption\_appendix.tex}
is read-only; this document was produced as a separate sibling file
\code{00\_assumption\_appendix\_v2.tex}. No modifications were made to v1.0.

\textbf{10.\ Absence of changelog content:} This document contains no
version-comparison section, no diff, and no migration notes. It reads as a
