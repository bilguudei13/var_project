"""
sweep_window_length.py
======================
Grid-search over rolling-window sizes for the GARCH(1,1)-t copula VaR model.

Runs mc_garch_t_copula with three different WINDOW values in parallel (one
process per window) and prints a side-by-side comparison of Kupiec +
Christoffersen backtest results.

Usage
-----
    python src/var_methods/sweep_window_length.py

Candidate windows (trading days):
    250  ~1 year   — more reactive, less stable parameter estimates
    500  ~2 years  — lecture-specified baseline
    750  ~3 years  — more stable, slower to react to regime changes

All other settings (M, ALPHA, REFIT_EVERY, EWMA_LAMBDA, SEED) are inherited
from mc_garch_t_copula.py unchanged.
"""
...
# =============================================================================
# WORKER  — runs the full rolling loop for one window size
# Called in a separate process by ProcessPoolExecutor
# =============================================================================

def _run_one_window(window: int) -> dict:
    """
    Full walk-forward GARCH-copula VaR loop for a given window size.
    Returns a dict of backtest statistics.
    """
    ...
    for t in range(window, T):
        need_refit = (omega_arr is None) or ((t - window) % v2.REFIT_EVERY == 0)
...
def main():
    print("=" * 70)
    print("  WINDOW SWEEP  —  GARCH(1,1)-t Copula MC VaR  (v2_audit core)")
    print(f"  Windows: {WINDOWS_TO_TEST} trading days")
    print(f"  Running {len(WINDOWS_TO_TEST)} workers in parallel …")
    print("=" * 70)

    results = {}

    with ProcessPoolExecutor(max_workers=len(WINDOWS_TO_TEST)) as executor:
        futures = {executor.submit(_run_one_window, w): w for w in WINDOWS_TO_TEST}
        for future in as_completed(futures):
            w = futures[future]
            try:
                r = future.result()
                results[w] = r
                print(f"  Window={w:4d}d  done  "
                      f"({r['n_refits']} refits, {r['N']} exceptions, "
                      f"exc rate={r['exc_rate']:.2%})")
            except Exception as exc:
                print(f"  Window={w:4d}d  ERROR: {exc}")

    # ── Comparison table ──────────────────────────────────────────────────────
    alpha = 0.99
    print("\n")
    print("=" * 95)
    print(f"  {'':8}  {'':6}  {'Exp':>5}  {'Obs':>5}  {'Exc%':>6}  "
          f"{'Kupiec LR':>10}  {'Kup p':>7}  {'Kup?':>5}  "
          f"{'Ind p':>7}  {'CC p':>7}  {'Mean VaR':>10}")
    print(f"  {'Window':8}  {'T_bt':>6}  {'Exc':>5}  {'Exc':>5}  {'':>6}  "
          f"{'':>10}  {'':>7}  {'Rej':>5}  "
          f"{'':>7}  {'':>7}  {'':>10}")
    print(f"  {'-'*88}")
    for w in sorted(results):
        r  = results[w]
        expected = r["T_bt"] * (1 - alpha)
        marker = " <-- baseline" if w == 500 else ""
        print(f"  {w:>5}d ({w//252:.0f}y)  "
              f"{r['T_bt']:>6}  "
              f"{expected:>5.1f}  "
              f"{r['N']:>5}  "
              f"{r['exc_rate']:>5.2%}  "
              f"{r['lr_uc']:>10.4f}  "
              f"{r['pvalue_uc']:>7.4f}  "
              f"{'YES' if r['reject_uc'] else 'no ':>5}  "
              f"{r['pvalue_ind']:>7.4f}  "
              f"{r['pvalue_cc']:>7.4f}  "
              f"${r['mean_var']:>9,.0f}"
              f"{marker}")
